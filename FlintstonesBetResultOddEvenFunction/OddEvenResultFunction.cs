...
namespace FlintstonesBetResultOddEvenFunction
{
    public class OddEvenResultFunction
    {
        [FunctionName("OddEvenResultFunction")]
        public async Task Run([ServiceBusTrigger("bets-odd-even", Connection = "odd-even-conn")]string myQueueItem, ILogger log)
        {
            ...
            var lastDigit = latestPrice.ToString().Last();

            if (bet.Selection == 3)
            {
                if (IsNumberEven(latestPrice) == false)
                {
                    // pay the client
            ...
            if (bet.Selection == 4)
            {
                if (IsNumberEven(latestPrice) == true)
                {
                    // pay the client
                    resultEntity.WinAmount = bet.TotalPayout;
                    bet.StatusID = 2;
        ...
        public static bool IsNumberEven(double price)
        {
            var lastNumber = price.ToString().Last();
